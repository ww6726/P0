...
void send_data(prime_field::field_element share,string pipe_name){
...
prime_field::field_element recv_data(string pipe_name)
{
	const char* pipeName = pipe_name.c_str();
...
{	
	...
	// if(prover_id==BOB){
	// 	send_data(share,ab);
	// 	share_merge = recv_data(bc);

	// }else if(prover_id==ALICE){
	// 	prime_field::field_element share_bob = recv_data(ab);
	// 	prime_field::field_element share_bob_alice = share + share_bob;
	// 	send_data(share_bob_alice,ac);
	// 	share_merge = recv_data(ac);

	// }else{
	// 	prime_field::field_element share_bob_alice = recv_data(ac);
	// 	share_merge = share + share_bob_alice;
	// 	send_data(share_merge,bc);
	// 	send_data(share_merge,ac);

	// }

	//network
	prime_field::field_element share_1 = recv_data(clients[0]);
	prime_field::field_element share_2 = recv_data(clients[1]);
	prime_field::field_element share_merge = share + share_1 + share_2;
	send_data(share_merge, clients[0]);
	send_data(share_merge, clients[1]);
